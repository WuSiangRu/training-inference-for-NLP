    ...
    if domain == "hotel":
        if domain in target_beliefs[t]:
            if "type" in pred_beliefs[domain] and "type" in target_beliefs[t][domain]:
                ...
                "type" in pred_beliefs[domain]
                and "type" not in target_beliefs[t][domain]
            ):
...
class MultiWozEvaluator(BaseEvaluator):
    def __init__(self, data_name):
        ...
        #     open("resources/multi-woz-2.1/delex.json", "r")
        # )
        self.delex_dialogues = json.load(open(r"resources/multi-woz/delex.json", "r"))
        self.db = MultiWozDB()
        self.labels = list()
        self.hyps = list()
                    ...
                    if domain in ["restaurant", "hotel", "attraction", "train"]:
                        ...
                        if domain not in pred_beliefs:
                            ...
                            venues = self.db.queryResultVenues(
                                domain, pred_beliefs[t][domain], real_belief=True
                            )
    ...
    def evaluateModel_gpt2(self, dialogues, real_dialogues=False, mode="valid"):
        ...
        report += (
            "{} Corpus Combined score : {:2.4f}%".format(
                mode,
                blue_score * 100
                + 0.5
                * ((matches / float(total) * 100) + (successes / float(total) * 100)),
            )
            + "\n"
        )
...
def postprocess_gpt2(generated_raw_data):
    ...
    for key, value in generated_raw_data.items():
        ...
        for bs in value["generated_belief"]:
            ...
            slot = bs.split()[1]
            val = " ".join(bs.split()[2:])
            if val in ["none", "dontcare", "not mentioned"]:
                continue
        ...
        if aggregated_belief_dict != belief_dict[-1]:
            for domain, value_ in aggregated_belief_dict.items():
                if (
                    domain == "attraction"
                    and domain in belief_dict[-1]
                    and len(aggregated_belief_dict[domain].keys())
                    < len(belief_dict[-1][domain].keys())
                ):
                    aggregated_belief_dict[domain] = belief_dict[-1][domain]
                elif (
                    domain == "restaurant"
                    and domain in aggregated_belief_dict
                    and "name" in aggregated_belief_dict[domain]
                    and aggregated_belief_dict[domain]["name"] == "lovell lodge"
                    and domain in belief_dict[-1]
                    and "name" in belief_dict[-1][domain]
                    and belief_dict[-1][domain]["name"] == "restaurant 17"
                ):
                    # ipdb.set_trace()
                    aggregated_belief_dict[domain] = belief_dict[-1][domain]
                elif (
                    domain == "restaurant"
                    and "name" in value_
                    and aggregated_belief_dict[domain]["name"] == "curry garden"
                    and "area" in aggregated_belief_dict[domain]
                    and aggregated_belief_dict[domain]["area"] == "east"
                    and domain in belief_dict
                ):
                    aggregated_belief_dict[domain] = belief_dict[-1][domain]
...
if __name__ == "__main__":
    ...
    eval_filename = "../custom_trainer_samuel/en_baseline_end2end_context[history=full_history].json"
    #eval_filename = "delex_en_baseline_context[history=full_history].json"
    # with open("resources/test_dials_lexicalized.json", "r") as f:
    # with open("resources/test_dials_2.1.json", "r") as f:
    with open(r"resources/test_dials.json", "r") as f:
