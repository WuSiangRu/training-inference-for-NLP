...
def normalize(text):
    ...
    # replace 一個 -> 1個
    values = ["一個", "兩個", "三個", "四個", "五個", "六個", "七個", "八個", "九個", "十個"]
    value_maps = {
        "一個": "1個",
        "兩個": "2個",
        "三個": "3個",
    ...
    for value in values:
        if value in text:
            text = text.replace(value, value_maps[value])
    return text
...
def normalize_for_sql(text):
    ...
    text = text.replace(
        "city centre north bed and breakfast", "city centre north b and b"
    )
    ...
    text = text.replace(
        "cherry hinton village centre", "the cherry hinton village centre"
    )
    ...
    text = text.replace(
        "cambridge contemporary art museum", "cambridge contemporary art"
    )
