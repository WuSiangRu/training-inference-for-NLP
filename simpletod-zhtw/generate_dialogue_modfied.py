...
model_checkpoint = r"simpletod_specialadded_rl_05"
#model_checkpoint = r"../dst_output"
...
break_tokens = tokenizer.encode(
    tokenizer.sep_token, add_special_tokens=False, max_length=512, truncation=True
)  # "[SEP]": 102
...
for i, dial_name in enumerate(tqdm(lex_dict)):
    ...
    for turn_id, (usr_turn, _) in enumerate(zip(user, system)):
        ...
        indexed_tokens = tokenizer.encode(text, add_special_tokens=False, max_length=512, truncation=True)
                ...
                if decoding == "nucleus":
                    ...
                    while predicted_index not in break_tokens:
                        ...
                        if "<|endofaction|>" in predicted_text and not truncate_action:
                            ...
                            indexed_tokens = tokenizer.encode(
                                predicted_text, add_special_tokens=False, max_length=512, truncation=True
                            )
                            truncate_action = True
                        # EndOfModified
