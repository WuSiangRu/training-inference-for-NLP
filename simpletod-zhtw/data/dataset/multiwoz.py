...
class MultiWozDataset:
    def __init__(self, args, split="train", shuffle=True):
        ...
        if args.lexical:
            file_path = os.path.join(
                self.data_dir, "{}_dials_lexicalized.json".format(split)
            )
        else:
            file_path = os.path.join(self.data_dir, "{}_dials.json".format(split))

        if args.no_history:  # if use current turn only
            if args.lexical:
        ...
        self.dialogues = json.load(open(file_path, "rt", encoding="UTF-8"))
        self.actions = json.load(
            open("resources/multi-woz/dialogue_acts.json", "rt", encoding="UTF-8")
        )

        self.input_word2index = json.load(
            open(input_word2index_filepath, "rt", encoding="UTF-8")
        )
        self.output_word2index = json.load(
            open(output_word2index_filepath, "rt", encoding="UTF-8")
        )
        self.input_index2word = json.load(
            open(input_index2word_filepath, "rt", encoding="UTF-8")
        )
        self.output_index2word = json.load(
            open(output_index2word_filepath, "rt", encoding="UTF-8")
        )

        # special tokens
        self.sos_token = 101  # CLS
        self.bos_token = 101  # CLS
        self.eos_token = 102  # SEP
        self.unk_token = 100  # UNK
        self.pad_token = 0
        ...
        if os.path.isfile(cached_filename) and not args.no_cached:
            ...
            for name in dial_names:
                ...
                for idx, (usr, sys, bs, db, bstate, sys_act) in enumerate(
                    ...
                    tensor = [
                        self.input_word2index[word]
                        for word in usr.strip(" ").split(" ")
                    ] + [EOS_token]
                    input_tensor.append(torch.LongTensor(tensor))  # .view(-1, 1))

                    tensor = [
                        self.output_word2index[word]
                        for word in sys.strip(" ").split(" ")
                    ] + [EOS_token]
    ...
    def pad_dialogue_with_history(self, dial, name):
        ...
        for i, (inp, tgt) in enumerate(zip(input, target)):
            if i == 0:
                ...
                i > self.args.history_length
            ):  # default=5, number of turns for context history
    ...
    def __getitem__(self, item):
        ...
        # print("ret_dial_name:", ret_dial_name) # ret_dial_name: SNG01856.json
        if self.args.no_history:
            dial = self.pad_dialogue(self.data[ret_dial_name], ret_dial_name)
        else:
            dial = self.pad_dialogue_with_history(
                self.data[ret_dial_name], ret_dial_name
            )
        dial["name"] = ret_dial_name
        return dial


class MultiWozDatasetDSTC9:
    def __init__(self, args, split="test", shuffle=True):
        self.args = args
        self.split = split
        self.data_dir = args.data_dir
        self.batch_size = args.batch_size
        if args.lexical:
            file_path = os.path.join(
                self.data_dir, "dstc9_{}_dials_lexicalized.json".format(split)
            )
        else:
            file_path = os.path.join(self.data_dir, "dstc9_{}_dials.json".format(split))

        if args.no_history:  # if use current turn only
            if args.lexical:
                input_word2index_name = "dstc9_input_lang.word2index_lexicalized.json"
                output_word2index_name = "dstc9_output_lang.word2index_lexicalized.json"
                input_index2word_name = "dstc9_input_lang.index2word_lexicalized.json"
                output_index2word_name = "dstc9_output_lang.index2word_lexicalized.json"
            else:
                input_word2index_name = "dstc9_input_lang.word2index.json"
                output_word2index_name = "dstc9_output_lang.word2index.json"
                input_index2word_name = "dstc9_input_lang.index2word.json"
                output_index2word_name = "dstc9_output_lang.index2word.json"
        else:
            if args.lexical:
                input_word2index_name = "dstc9_history_lang.word2index_lexicalized.json"
                output_word2index_name = (
                    "dstc9_history_lang.word2index_lexicalized.json"
                )
                input_index2word_name = "dstc9_history_lang.index2word_lexicalized.json"
                output_index2word_name = (
                    "dstc9_history_lang.index2word_lexicalized.json"
                )
            else:
                input_word2index_name = "dstc9_history_lang.word2index.json"
                output_word2index_name = "dstc9_history_lang.word2index.json"
                input_index2word_name = "dstc9_history_lang.index2word.json"
                output_index2word_name = "dstc9_history_lang.index2word.json"
        ...
        self.dialogues = json.load(open(file_path, "rt", encoding="UTF-8"))
        self.actions = json.load(
            open("zhtw_dstc9_dialogue_acts.json", "rt", encoding="UTF-8")
        )
        ...
        if args.lexical:
            cached_filename = "resources/dstc9_cached_data_lexical_{}.pkl".format(split)
        else:
            cached_filename = "resources/dstc9_cached_data_delex_{}.pkl".format(split)
