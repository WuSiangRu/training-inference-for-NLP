...
class LineByLineTextDataset_shuffle_context(Dataset):
    ...
    def shuffle_context(self, text):
        context, after_context = (
            text.strip().split("<|context|>")[-1].split("<|endofcontext|>")
        )
        ...
        if after_context != "":
            new_text = "<|endoftext|> <|context|> {} <|endofcontext|> {}".format(
                shuffled_context, after_context
            )
        else:
            new_text = "<|endoftext|> <|context|> {} <|endofcontext|>".format(
                shuffled_context
...
class LineByLineTextDataset_shuffle_belief(Dataset):
    ...
    def shuffle_belief(self, text):
        ...
        belief, after_belief = (
            text.strip().split("<|belief|>")[-1].split("<|endofbelief|>")
        )
        tmp = belief.split(",")
        random.shuffle(tmp)
        new_belief = ",".join(tmp)
        new_text = "{} <|belief|> {} <|endofbelief|> {}".format(
            before_belief, new_belief, after_belief
        )
    ...
    def shuffle_action(self, text):
        ...
        action, after_action = (
            text.strip().split("<|action|>")[-1].split("<|endofaction|>")
        )
        tmp = action.split(",")
        random.shuffle(tmp)
        new_action = ",".join(tmp)
        new_text = "{} <|action|> {} <|endofaction|> {}".format(
            before_action, new_action, after_action
        )
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if not evaluate:
        if args.shuffle_context:
            return LineByLineTextDataset_shuffle_context(
                tokenizer, args, file_path=file_path, block_size=args.block_size
            )
        elif args.shuffle_belief_action:
            return LineByLineTextDataset_shuffle_belief_action(
                tokenizer, args, file_path=file_path, block_size=args.block_size
            )
        elif args.shuffle_belief:
            return LineByLineTextDataset_shuffle_belief(
                tokenizer, args, file_path=file_path, block_size=args.block_size
            )
        else:
