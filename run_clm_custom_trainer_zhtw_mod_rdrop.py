...
def main():
    ...
    if model_args.model_name_or_path:
        ...
        print("model相關配置:", model.config.resid_pdrop)
    else:
    ...
    with training_args.main_process_first(desc="dataset map tokenization"):
        ...
        tokenized_datasets.remove_columns_(
            ["token_type_ids"]
        )  # 經過BERT分詞器處理後，會有token_type_ids，但中文GPT-2模型不需要，移除他。
    ...
    BS_MAX_LEN = 251  # belief state 最長長度 251
    model.config.eos_token_id = (
        21131  # For RL training，設定模型只生成到 <|endofbelief|> 的ID: 21131
    )
    ...
    class RTCTODTrainer(Trainer):
        ...
        def greedy(self, batch):
            ...
            # for LEN in input_ids:
                # print("input_ids_LEN", len(LEN))
            input_text_batch = tokenizer.batch_decode(
                input_ids, skip_special_tokens=False
            )
            print("input_text_batch:", input_text_batch)
            # sys.exit()
            del input_ids
            ...
            for input_text in input_text_batch:
                ...
                # print("=target_belief_state=:", target_belief_state)
                target_belief_states.append(target_belief_state)
                # print("==target_belief_states==:", target_belief_states)
            target_belief_states = ["".join(bs.split()) for bs in target_belief_states]###
            print("===target_belief_states===:", target_belief_states)
            history_text_batch = [
                input_text.split("<|belief|>")[0] + "<|belief|>"
                for input_text in input_text_batch
            ]###
            print("===history_text_batch:", history_text_batch)
            ...
            print("beam_outputs", (beam_outputs.sequences))
            ...
            model.generate 設定的 pad_token_id=21131，來去作padding，以確保所有句子最後長度都一樣。
            pad_token_id=21131，是我們在中文GPT-2的字典額外新增的<|endofbelief|> ID ，這邊的想法是
            model.generate會把提早生成出belief state的語句利用 "<|endofbelief|>" 進行padding，
            因此某些句子會呈現 dialogue_history -> belief_state -> "<|endofbelief|> <|endofbelief|> <|endofbelief|> ..." 生成一堆 "<|endofbelief|>" 來補齊長度。

            如果之後有自訂義的token id要停下來，請把程式裡面所有出現21131的地方進行修改。

            因此，以下的程式我們要將每個生成句多餘的 "<|endofbelief|>" 清除。
            """
            batch_logits = torch.stack(beam_outputs.scores, dim=1).to("cpu")
            # for logits in batch_logits:
            #     print(logits.shape)  # [number_of_generated_token, 50258 字典大小]

            # 抓出每個句子真正的end index，也就是抓到第一個出現的 <|endofbelief|> 的才是真正的 end token
            batch_eos_index = [
        ...
        def reinforce(
            ...
            for avg, accs, logits, actions in zip(
                ...
                for acc, l, a in zip(accs, logits, actions):
                    ...
                    print("net_policies", net_policies)
                    for i in net_policies:
                        print(i.shape)
                    # print("net_policies's shape", net_policies)
            ...
            print("policies_v", policies_v)
            print("policies_v's shape", len(policies_v))
        ...
        def compute_loss(self, model, inputs, return_outputs=False):
            ...
            if 1.0 - alpha:
                ...
                for i in outputs.logits:
                    print(i)
                print("size", outputs.logits.size())
                print("ce_loss:", outputs.loss)
                sys.exit()
                ce_loss = outputs.loss
